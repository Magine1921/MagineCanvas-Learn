...
中转站与官方版使用相同的全能参考配置和请求参数，区别仅在 API 地址与鉴权密钥。

```text
API 地址: https://api.tokenriver.cn
```

支持标准版与 Fast 版，可同时引用最多 9 张图片、3 段视频和 3 段音频。参考素材会分别按
`reference_image`、`reference_video`、`reference_audio` 角色提交，时长、比例、分辨率、
同步音频、尾帧返回和水印设置与官方版保持一致。
